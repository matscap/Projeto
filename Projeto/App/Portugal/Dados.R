myReactiveDataPais <- reactive({
  ...
  Vac = unlist(rep(c("Comirnaty","Moderna","AstraZeneca", "Janssen"),6))
  Doses=round(unlist(c(sum(COM_18_24Vacinados['SecondDose']),
...
myReactiveDataPais3 <- reactive({
  ...
  Percentagem_tpv4 = 100 - Percentagem_tpv3
  
  teste = TotalCompVacinados
  teste2 = totalPopulacao - TotalCompVacinados
  ##Percentagem_tpNv = ((totalPopulacao-TotalCompVacinados)/totalPopulacao)*100
  
  vals <- c(round(Percentagem_tpv4), round(Percentagem_tpv3))
  ##vals2 <-c(teste,teste2)
})
