...
shinyServer(
  function(input,output,session){
    ...
    ##########################################################################
    # Rafa- Vou testar uma cena de mapas
      # Estou a usar as packages geojsonio e leaflet

    pt <- geojsonio::geojson_read("www/Portugal.json", what = "sp")
    

    output$myMap <- renderLeaflet({
      leaflet(pt) %>% 
        addTiles() %>%
          #fitBounds(~min(-31.368122), ~min(32.479751), ~max(-6.173116), ~max(42.361281))%>%
          setView( lng = -19.014070,lat = 38.234966, zoom = 5.5) %>%
            addProviderTiles("CartoDB.Positron", options = providerTileOptions(noWrap = TRUE)) %>%
            addPolygons()
        
       # addPolygons(  
        #  weight = 0.7,  # line thickness
         # opacity = 0.7,  # line transparency
          #color = "black",
          #fillOpacity = 0.7,
          
          #fillColor = ifelse(
    #        test = pt@data[["region"]] =="Lisbon",
     #       yes = "red",
      #      no = ifelse(
       #       test = pt@data[["region"]] =="Azores",
        #      yes = "green",
         #     no = ""
          #  )
          #)
